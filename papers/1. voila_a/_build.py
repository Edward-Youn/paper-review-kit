...
def render_diss_card(c):
    ...
    if c.get("cls") == "diss-summary" and "dissection_overview" in GEN_DATAURI:
        overview_html = (
            ...
            f'<img src="{GEN_DATAURI["dissection_overview"]}" alt="Voila-A 한 장 정리" />'
            '<figcaption>PROBLEM &rarr; OBSERVATION &rarr; METHOD &rarr; NOVELTY &rarr; RESULTS &mdash; 5단 가로 흐름으로 본 Voila-A. 본문 9-row와 동일 내용의 시각 요약.</figcaption>'
            '</figure>'
        )
    return (
...
tab_knowledge = (
    ...
    '<p>Voila-A를 이해하기 위한 사전 지식 빌딩 블록 + 핵심 수식 + 개념 카드. VLM/MLLM · 어텐션/크로스 어텐션 · Perceiver Resampler · OpenFlamingo(Otter) · 시선 추적/saliency · Localized Narratives 트레이스 → 그 위에 시선 히트맵을 어텐션 정보 응집자로 주입하는 Voila-A의 신개념.</p>'
    '</div>'
    ...
    '<h3 class="panel-title">핵심 수식</h3>'
    '<p class="panel-sub">가우시안 시선 히트맵 (1) · 트레이스↔시선 정렬을 재는 EMD (2) · Voila Perceiver Block (3) · 시선 G를 K에만 더하는 Voila 어텐션 (4) · 히트맵 패치 인코딩 (6) — Voila-A의 정량 골격.</p>'
    f'<div class="eq-grid eq-grid-detailed">{eq_cards_html}</div>'
    '</section>'
    '<section class="fund-panel">'
    '<h3 class="panel-title">개념 카드</h3>'
    '<p class="panel-sub">본 논문의 핵심 개념 — 마우스 트레이스를 시선의 proxy로 · 히트맵 vs 이산 토큰/bbox 인코딩 · 어텐션 정보 응집자로서의 시선 · 파국적 망각과 점진적 unfreezing · GPT-4 Ranking + reward model 평가 · coreference 질의 · VOILA-COCO/VOILA-GAZE · [fixation] 토큰.</p>'
...
tab_questions = (
    ...
    '<p>본문이 명시하지 않은 가정 · 흔한 오해 · 비판적 질문 · 확장 가능성을 4개 카드로 분해. 트레이스≈시선 가정 · 히트맵 vs bbox 오해 · GPT-4 심판의 편향 · 음성/고해상도 OCR 확장까지.</p>'
    '</div>'
...
(function(){
  ...
  document.addEventListener('click', e => { if(!modal.classList.contains('open')) return; if(modal.contains(e.target)) return; if(e.target.closest('.study-fab')) return; modal.classList.remove('open'); });
...
{tab_qa}
  ...
  <footer class="foot">
    <p>Paper Review HTML · v3 · Voila-A (arXiv 2401.09454) — VLM에 사용자의 시선(gaze)을 정렬시키다</p>
  </footer>
</main>
...
OUT_HTML = ROOT / "VoilaA_output.html"
OUT_HTML.write_text(HEAD + BODY, encoding="utf-8")
size_mb = OUT_HTML.stat().st_size / (1024*1024)
print(f"=== VoilaA_output.html built: {size_mb:.2f} MB ===")
