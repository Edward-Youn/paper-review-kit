...
- 빌더 정본 (대화형 빌드 + 외부 데이터 JSON → 단일 HTML 조립 모범) — `papers/1. voila_a/VoilaA_output.html` + `_build.py` (및 워크드 예제 `samples/free_example/_build.py`)
...
정본 구현 = `papers/1. voila_a/_crop.py` (+ 캡션 좌표 검출 도구 `tools/detect_assets.py`). 자세한 절차·검출 코드·column 좌표 표·자산 유형별 컨벤션: `rules/parsing_rules.md` §4-A "vector PDF — 캡션 좌표 기반 bbox 검출".
...
- 정본 사례: `papers/1. voila_a/_build.py`
